import { memo, forwardRef } from 'react';
import { IconBase } from '../base.js';
import type { IconBaseProps } from '../base.js';

type MessageMessagesFillDuotoneProps = Omit<IconBaseProps, 'children'>;

const MessageMessagesFillDuotone = memo(
  forwardRef<SVGSVGElement, MessageMessagesFillDuotoneProps>((props, ref) => (
    <IconBase ref={ref} {...props}>
      <path d="M19 7.63q.68 0 1.15.02.49.02.95.2c.7.29 1.26.85 1.56 1.55q.17.45.2.95.02.47.01 1.15V21a.88.88 0 0 1-1.33.74l-3.8-2.36H10.5q-.62.02-1.06-.06a2.9 2.9 0 0 1-2.26-2.26q-.07-.44-.06-1.06a.88.88 0 0 1 1.75 0c0 .5 0 .63.03.72.09.45.43.8.88.88.09.02.21.02.72.02H18q.24 0 .46.14l2.66 1.66V11.5l-.01-1.04a1 1 0 0 0-.07-.4q-.19-.41-.6-.6a1 1 0 0 0-.4-.07L19 9.37a.88.88 0 0 1 0-1.74" opacity={.4} />
        <path d="M12.8 2.13q.82 0 1.38.03.6.03 1.13.28.83.42 1.25 1.25.25.54.28 1.13.05.56.04 1.38v3.6q.01.82-.04 1.38-.03.6-.28 1.13-.42.83-1.25 1.25-.54.25-1.13.28-.55.05-1.38.04H6.25l-3.79 2.36a.88.88 0 0 1-1.33-.74V6.2q0-.82.03-1.38.03-.6.28-1.13.42-.83 1.25-1.25.54-.25 1.13-.28.56-.05 1.38-.04z" />
    </IconBase>
  ))
);

MessageMessagesFillDuotone.displayName = 'MessageMessagesFillDuotone';

// Triple export pattern (lucide-react style)
export { MessageMessagesFillDuotone, MessageMessagesFillDuotone as MessageMessagesFillDuotoneIcon, MessageMessagesFillDuotone as SiMessageMessagesFillDuotone };
export default MessageMessagesFillDuotone;
export type { MessageMessagesFillDuotoneProps };
